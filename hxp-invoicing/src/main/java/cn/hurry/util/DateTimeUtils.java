...
public final class DateTimeUtils {
	 ...
	 */
	public static String compareDaysWithDate(Date date, Date date2, boolean bh) {
		StringBuilder msg = new StringBuilder();
		DateTime startDate = new DateTime(date);
		DateTime endDate = new DateTime(date2);
		if (startDate.getDayOfMonth() != endDate.getDayOfMonth()) {
			startDate.withMinuteOfHour(0);
			endDate.withMinuteOfHour(0);
		}
		int minutes = (int) Math.abs((startDate.toDate().getTime() - endDate.toDate().getTime()) / 60000);
		if (startDate.getDayOfYear() == endDate.getDayOfYear()) {
			msg.append(minutes / 60 + "小时");
		} else if (minutes / 60 / 24 > 0) {
			msg.append((minutes / 60 / 24 + (bh ? 1 : 0)) + "天");
	...
	public static Date formatUnknownString2Date(String dateString) {
		try {
			...
			// 切割年月日
			String yearStr = StringUtil.isNumber(dateString) ? dateString : "";
			String monthStr = "";
			String dateStr = "";
			// 截取日期
			...
			// 返回日期
			return new DateTime(Integer.parseInt(yearStr.trim()), Integer.parseInt(monthStr.trim()), Integer.parseInt(dateStr.trim()), Integer
					.parseInt(hmsarr[0].trim()), Integer.parseInt(hmsarr[1].trim()), Integer.parseInt(hmsarr[2].trim()), 0).toDate();
		} catch (Exception e) {
			logger.error("解析多种格式的时间字符串错误", e);
			return null;
		}
	}
	 ...
	 *         例1：调用{@link DateTimeUtils.getDatesByString("2012.09.10-2015.05.20", ".", "-")} <br>
